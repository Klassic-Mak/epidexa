...
## Notes & tips
- The client conatins the files such as model client and etc
- There are multiple `pubspec.yaml` files — run `dart/ flutter pub get` in the specific package folder you are working in.
- If you see `build/` or generated folders, they can usually be removed and regenerated by building the project again.
- Check each project's own README (if present) for project-specific instructions.

---

If you want, I can:
- Add a short README into `skinAware_server/`, `skinAware_flutter/`, and `skinAware_client/` with run/debug steps specific to each.
- Create a `.gitignore` (or update it) to exclude `build/` and other generated files.

If you'd like any changes to the wording or more detail for any folder, tell me which folders to expand.
